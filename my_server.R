...
my_server <- function(input, output, session) {
  ...
  output$description_model <- renderText({
    message <- paste0(("Statistics for the "),
                      input$car_brand, " ", input$car_model)
    message
  })
  ...
  output$ranking_description <- renderText({
    ...
    above or below average the selected car is in relation to all other
    cars in our data. If the number shown is worse than the average value
    of all other cars, then the column will be shown in red. If the
    number shown is better than the average value of all other cars,
    then it will be shown in blue. For example, a lower MPG and higher gas
    consumption will both be shown as red on the graph, and higher gas
    savings and lower fuel costs will both be shown in blue. "
  })
